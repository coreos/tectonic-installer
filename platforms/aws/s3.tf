...
resource "aws_s3_bucket" "tectonic" {
  ...
  bucket = "${var.tectonic_s3_bucket_name == "" ? format("%s%s-%s", "a", var.tectonic_cluster_name, md5(format("%s-%s", data.aws_region.current.name , var.tectonic_base_domain))) : var.tectonic_s3_bucket_name }"
