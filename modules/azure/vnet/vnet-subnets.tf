...
resource "azurerm_subnet" "master_subnet" {
  ...
  resource_group_name       = "${var.external_vnet_id == "" ? var.resource_group_name : replace(var.external_vnet_id, "${local.const_id_to_group_name_regex}", "$1")}"
  virtual_network_name      = "${var.external_vnet_id == "" ? join("",azurerm_virtual_network.tectonic_vnet.*.name) : replace(var.external_vnet_id, "${local.const_id_to_group_name_regex}", "$2")}"
