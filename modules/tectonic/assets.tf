...
resource "template_dir" "tectonic" {
  ...
  vars {
    ...
    // We need this hack to generate a conditional yaml block
    // for storage classes, since the extra parameters differ
    // from platform to platform
    storage_parameters = "${var.tectonic_storage_params ? "parameters:\n  ${indent(2, join("\n", var.tectonic_storage_params_list))}" : ""}"
