...
| tectonic_etcd_tls_enabled | (optional) If set to `true`, TLS secure communication for self-provisioned etcd. If `tectonic_experimental` is set to `true` this variable has no effect, because the experimental self-hosted etcd always uses TLS.<br><br>Note: This is variable has no effect if `tectonic_experimental` is set to `true`. | string | `true` |
